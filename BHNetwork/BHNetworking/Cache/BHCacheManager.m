...
@implementation BHCacheManager

+ (BHCacheManager *)shareManager {
    static BHCacheManager *_BHCacheManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _BHCacheManager = [[BHCacheManager alloc] init];
    });
    return _BHCacheManager;
}
...
- (void)cacheResponseObject:(id)responseObject
    ...
    if (error == nil) {
        //缓存到内存中
        [BHMemoryCache writeData:responseObject forKey:hash];
        ...
        if (!directoryPath) {
            directoryPath = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES)[0] stringByAppendingPathComponent:@"BHNetworking"] stringByAppendingPathComponent:@"networkCache"];
            [[NSUserDefaults standardUserDefaults] setObject:directoryPath forKey:cacheDirKey];
            [[NSUserDefaults standardUserDefaults] synchronize];
        }
        [BHDiskCache writeData:data toDir:directoryPath filename:hash];
        
        [[BHLRUManager shareManager] addFileNode:hash];
    }
...
- (id)getCacheResponseObjectWithRequestUrl:(NSString *)requestUrl
    ...
    //先从内存中查找
    cacheData = [BHMemoryCache readDataWithKey:hash];
    
    if (!cacheData) {
        NSString *directoryPath = [[NSUserDefaults standardUserDefaults] objectForKey:cacheDirKey];
        
        if (directoryPath) {
            cacheData = [BHDiskCache readDataFromDir:directoryPath filename:hash];
            
            if (cacheData) [[BHLRUManager shareManager] refreshIndexOfFileNode:hash];
        }
    }
...
- (void)storeDownloadData:(NSData *)data
    ...
    if (!directoryPath) {
        directoryPath = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES)[0] stringByAppendingPathComponent:@"BHNetworking"] stringByAppendingPathComponent:@"download"];
    ...
    [BHDiskCache writeData:data toDir:directoryPath filename:fileName];
...
- (NSURL *)getDownloadDataFromCacheWithRequestUrl:(NSString *)requestUrl {
    ...
    if (directoryPath) data = [BHDiskCache readDataFromDir:directoryPath filename:fileName];
...
- (NSUInteger)totalCacheSize {
    ...
    return [BHDiskCache dataSizeInDir:diretoryPath];
}
...
- (void)clearDownloadData {
    ...
    [BHDiskCache clearDataIinDir:diretoryPath];
}
...
- (void)clearTotalCache {
    ...
    [BHDiskCache clearDataIinDir:directoryPath];
}

- (void)clearLRUCache {
    if ([self totalCacheSize] > diskCapacity) {
        NSArray *deleteFiles = [[BHLRUManager shareManager] removeLRUFileNodeWithCacheTime:cacheTime];
        NSString *directoryPath = [[NSUserDefaults standardUserDefaults] objectForKey:cacheDirKey];
        if (directoryPath && deleteFiles.count > 0) {
            [deleteFiles enumerateObjectsUsingBlock:^(NSString *obj, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                [BHDiskCache deleteCache:filePath];
            }];
